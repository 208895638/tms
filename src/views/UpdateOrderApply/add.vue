  ...
  <div class="app-container a1">
    ...
    <el-table class="edit-page-modular" 
      ...
      <el-table-column label="货物编码" >
        <template slot-scope="scope">
          <el-input  class="input-group" disabled v-model="scope.row.m.Goods_Code"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="名称" align="center" >
        <template slot-scope="scope">
          <el-input class="input-group" disabled v-model="scope.row.m.Goods_Name"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="包装" align="center" >
        <template slot-scope="scope">
          <el-input class="input-group" disabled v-model="scope.row.m.Goods_Package"></el-input>
        </template>
      </el-table-column>
      ...
      <el-table-column label="件数"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" type="number" disabled @change="ListOperation(scope.$index)" v-model="scope.row.m.Goods_Piece"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="重量(公斤)"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" disabled @change="ListOperation(scope.$index)" v-model="scope.row.m.Goods_Weight"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="体积(立方)"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" disabled @change="ListOperation(scope.$index)" v-model="scope.row.m.Goods_Volume"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="运费单价"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" placeholder="0" disabled @change="ListOperation(scope.$index)" v-model="scope.row.m.Freight_Unitprice"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="运费金额"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" disabled @change="changeYF(scope.row)" v-model="scope.row.m.Freight_Totalprice"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="保险金额"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" disabled @change="ListOperation(scope.$index)"  v-model="scope.row.m.Insurance_Amount"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="保费比"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" disabled @change="ListOperation(scope.$index)"  v-model="scope.row.m.Premium_Ratio"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="保险费"  align="center">
        <template slot-scope="scope">
          <!-- <el-input class="input-group" disabled v-model="scope.row.consignor_phone"></el-input> -->
        </template>
      </el-table-column>
      <el-table-column label="备注"  align="center">
        <template slot-scope="scope">
          <el-input class="input-group" disabled v-model="scope.row.m.Manifestdtl_Comment"></el-input>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    postMessage() { //  提交运输委托单到后台
      ...
      if (this.result.tuoyuntime == '' || this.result.tuoyuntime == undefined || 
          this.result.Arrival_Org_ID == '' || this.result.Arrival_Org_ID == undefined || 
          this.result.Consigner_Phone == '' || this.result.Consigner_Phone == undefined || 
          this.result.Consignee_Phone == '' || this.result.Consignee_Phone == undefined || 
          this.result.HandoverMethod == '' || this.result.HandoverMethod == undefined || 
          this.result.payme_value == '' || this.result.payme_value == undefined || 
          this.result.Consigner_Contact == '' || this.result.Consigner_Contact == undefined) {
        this.$message({
        ...
        axios.post(url.apiUrl() + '/api/ManifestHistory/AddManifestHistory', data1)
          .then(function(val) {
            if (val.data.error == '') {
              _this.$message({
                type: 'success',
              ...
              //_this.fetchData();
              _this.$router.push({ path: '/operationFlow/updateOrderApply' })
            } else {
              _this.$message.error(val.data.error)
            }
          })
